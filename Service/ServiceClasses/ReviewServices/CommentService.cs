...
public class CommentService : ServiceBase<Comment, CommentResult, Guid>, ICommentService
{
    ...
    public CommentService(IBaseRepository<Comment, Guid> commentRepository, IUserService userService)
    {
    ...
    public async Task<PaginatedList<CommentResult>> GetAllUnConfirmComments(int pageIndex, int pageSize, CancellationToken cancellation)
    {
        ...
        var unConfirmComments =await _commentRepository.GetAllDataAsync(x=>new CommentResult {
        ...
        },cancellation,x=>x.IsConfirmed==1,x=>x.Include(x=>x.Customer).ThenInclude(x=>x.User).Include(x=>x.Product));
